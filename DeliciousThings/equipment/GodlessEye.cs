...
{
    public partial class GodlessEye : EquipmentDef, IStaticContent, INetworkedObjectPrefabProvider
    {
        ...
        public readonly float maxConsecutiveEnemies = EquipmentConfig.Bind(SECTION, "Maximum Consecutive Enemies", 10).Value;
        ...
        public void Awake()
        {
            if (enabled)
            {
                ...
                name = string.Format(IDENTIFIER_FORMAT, "DeathEye");
                AutoPopulateTokens();
                canDrop = true;
                isLunar = true;
                ...
                EquipmentActivationFunctions[this] = FireDeathEye;
            }
        ...
        public class Consumed : EquipmentDef, IStaticContent
        {
            public void Awake()
            {
                if (Instance)
                {
                    ...
                    name = string.Format(IDENTIFIER_FORMAT, "DeathEyeConsumed");
                    AutoPopulateTokens();
                    canDrop = false;
                    isLunar = true;
