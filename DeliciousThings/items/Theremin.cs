...
{
    public partial class Theremin : ItemDef, IStaticContent
    {
        ...
        public readonly float attackSpeedBonus = ItemsConfig.Bind(SECTION, "Attack Speed Bonus", 0.45f).Value;
        public readonly float attackSpeedBonusPerStack = ItemsConfig.Bind(SECTION, "Attack Speed Bonus Per Stack", 0.35f).Value;
        ...
        public void Awake()
        {
            if (enabled)
            {
                ...
                name = string.Format(IDENTIFIER_FORMAT, "Theremin");
                AutoPopulateTokens();
        ...
        private void MusicController_UpdateTeleporterParameters(On.RoR2.MusicController.orig_UpdateTeleporterParameters orig, MusicController self, TeleporterInteraction teleporter, Transform cameraTransform, CharacterBody targetBody)
        {
            ...
            if (targetBody && targetBody.HasItem(this) && targetBody.TryGetComponent(out ThereminBehaviour behaviour))
            {
        ...
        {
            var texThereminIcon = assets.LoadAssetAsync<Sprite>("texThereminIcon");
            var PickupTheremin = assets.LoadAssetAsync<GameObject>("PickupTheremin");
            var DisplayTheremin = assets.LoadAssetAsync<GameObject>("DisplayTheremin");

            yield return texThereminIcon;
            pickupIconSprite = (Sprite)texThereminIcon.asset;
            yield return PickupTheremin;
            pickupModelPrefab = (GameObject)PickupTheremin.asset;
            Ivyl.SetupModelPanelParameters(pickupModelPrefab, new ModelPanelParams(new Vector3(56, 180, 0), 1, 5));

            yield return DisplayTheremin;
            GameObject displayModelPrefab = (GameObject)DisplayTheremin.asset;
            Ivyl.SetupItemDisplay(displayModelPrefab);
            ItemDisplaySpec itemDisplay = new ItemDisplaySpec(this, displayModelPrefab);
            yield return IDRS;
            var idrs = IDRS.Result;
